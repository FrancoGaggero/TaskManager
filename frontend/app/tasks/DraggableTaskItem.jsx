  ...
  return (
    <>
      <li
        ...
        onDrop={handleDrop}
        className={`flex justify-between items-center bg-[#1E1E1E] p-4 rounded-xl shadow-lg border transition-all duration-200 ${
          isDragging 
            ? "opacity-50 rotate-3 scale-95" 
            : isDragOver
            ? "border-[#FFB347] border-t-4 mt-2"
            : "border-[#FFB347]/30 hover:border-[#FFB347]/50"
        } hover:shadow-[#FFB347]/10 ${canEdit ? "cursor-move" : "cursor-default"}`}
      >
        <div className="flex items-center gap-3 flex-1">
          {canEdit && (
            <span className="text-gray-400 text-lg cursor-move select-none">
              ⋮⋮
            </span>
          )}
